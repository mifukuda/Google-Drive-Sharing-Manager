...
export default function DeviantSharingCard(props) {
    ...
    let deviantPermissions = deviantFile.permissions.map((element, index) => <p key={index+1}>{index + 1}. {element.granted_to.email}, {element.granted_to.display_name} ({roles[element.role]})</p>);
    deviantPermissions.unshift(<p key={0}><b>Deviant Permissions</b></p>);
    return (
